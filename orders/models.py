...
class StandardStrMixin():
    """str repr showing all attributes seprated by ,"""
    def __str__(self):
        fields = [f'{field.name}: {getattr(self, field.name)}'
...
class MenuItem(models.Model):
    ...
    size = models.ForeignKey(Size, on_delete=models.CASCADE,
                             blank=True, null=True, related_name='menu_items')
    sort_order = models.IntegerField()
...
class MenuItemAddon(models.Model):
    ...
    allowed_for = models.ManyToManyField(MenuItem, blank=True,
                                         related_name='allowed_addons')
    sort_order = models.IntegerField()
...
class Address(models.Model):
    ...
    def __str__(self):
        return f''' {self.user}\'s address: {self.name} {self.address1}
                    {self.address2} {self.zip_code} {self.city}'''
...
class OrderItem(models.Model):
    ...
    @property
    def price(self):
        return self.menu_item.price + sum(addon.menu_item_addon.price
